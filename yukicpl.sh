...
adusr()(  ##添加一个新用户并配置数据库权限
    ...
    echo $(mysql -e "select distinct user from mysql.user;") | grep -q -E "$USERNAME_" && {
        USERNAME=user$RANDOM$RANDOM
    } || {
        USERNAME=$USERNAME_
    }
    ...
    echo "$SL1" | grep -q -E '^[Nn]$' && {
        ...
        mysql -e "grant select,insert,update,create,delete on $DATABASENAME.* to '$USERNAME'@'localhost' "
    }
    _flush
)
...
delsql()( ##手动移除一个数据库
    ...
    echo $(mysql -e 'show databases') | grep -q -E "$DATABASENAME" && {
        SL=""
...
_vpntest()(
    ...
    test $vpn_type = "sstp" && {
        ...
        read -e -p "要不要试试Shadowsocks-libev版？（Y/n）" SL
...
_check_nginx(){
    ...
    } >/dev/null || apt install -y  nginx-full php-fpm php-cgi php-curl php-gd php-mcrypt php-mbstring php-sqlite3 php-zip php-xml ##php7.0-mysql
    _check_mysql
}
